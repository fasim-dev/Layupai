...
function showModal(selector){ 
    ...
    if (modal) {
        ...
        // ✅ Fokus ke input API key ketika modal dibuka
        setTimeout(() => {
...
function setupApiKeyManagement() {
    console.log('setupApiKeyManagement called');
    const apiKeyBtn = $('#apiKeyBtn');
    const apiKeySave = $('#apiKeySave');
    const apiKeyInput = $('#apiKeyInput');
    const apiStatus = $('#apiStatus');
    
    console.log('API Key elements:', {apiKeyBtn, apiKeySave, apiKeyInput, apiStatus});
    
    // ✅ Update status API key
    function updateApiStatus() {
        if (apiStatus) {
            if (DEEPSEEK_API_KEY && DEEPSEEK_API_KEY !== 'your_deepseek_api_key_here') {
                apiStatus.innerHTML = '<span style="color:var(--success)"><i class="fa-solid fa-check-circle"></i> API Key terpasang</span>';
            } else {
                apiStatus.innerHTML = '<span style="color:var(--warning)"><i class="fa-solid fa-exclamation-circle"></i> API Key belum diatur</span>';
            }
        }
    }
    
    updateApiStatus(); // Set status awal
    ...
    if (apiKeyBtn) {
        apiKeyBtn.addEventListener('click', () => {
            ...
            if (apiKeyInput) {
                ...
                console.log('API Key input value set to:', DEEPSEEK_API_KEY);
                updateApiStatus(); // Update status ketika modal dibuka
            }
        });
    }
    ...
    if (apiKeySave && apiKeyInput) {
        apiKeySave.addEventListener('click', () => {
            ...
            console.log('Saving API Key:', newApiKey ? '***' + newApiKey.slice(-4) : 'empty');
            ...
            if (newApiKey) {
                ...
                updateApiStatus(); // Update status setelah save
...
async function testApiKey(apiKey) {
    try {
        ...
        if (response.ok) {
            showToast('API Key valid! Koneksi berhasil', 'success');
        } else {
            showToast('API Key tidak valid', 'error');
        }
    } catch (error) {
        console.error('API test failed:', error);
        showToast('Gagal menguji API Key', 'error');
    }
}
...
setupCloseModalHandlers();
    ...
    // DeepSeek API toggle - Pastikan ini di setupEventListeners()
const apiToggle = $('#apiToggle');
if (apiToggle) {
    console.log('API toggle found');
    
    // ✅ Set initial value correctly
    const useDeepSeek = localStorage.getItem('layup_useDeepSeek');
    console.log('Saved useDeepSeek value:', useDeepSeek);
    
    // Default to true if not set
    apiToggle.checked = useDeepSeek !== 'false';
    
    apiToggle.addEventListener('change', (e) => {
        const useApi = e.target.checked;
        console.log('API toggle changed to:', useApi);
        localStorage.setItem('layup_useDeepSeek', useApi);
        
        if (useApi) {
            if (!DEEPSEEK_API_KEY || DEEPSEEK_API_KEY === 'your_deepseek_api_key_here') {
                showToast('Aktifkan API Key di pengaturan', 'warning');
                showModal('#modalApiKey');
            } else {
                showToast('DeepSeek API diaktifkan', 'success');
            }
        } else {
            showToast('DeepSeek API dimatikan', 'info');
        }
    });
}
}
